<?php include_once("titel.php"); titel("Movies | Hobo"); keywords(""); include_once("header.php");
...
<article class='seriepop' id='seriepop'>
    ...
    <article class='toparticle'>
        ...
        $sql1 = "SELECT * FROM films ORDER BY id ASC";
        ...
        $sql2 = "SELECT * FROM films WHERE id = '$rannum'";
        ...
        <p>$date</p><br>
        <p>$desc</p>
        <button><a class='numClick' Data-id='$id' Data-type='film'>Watch!</a></button>
        </div>";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 5 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 3 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 6 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 8 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 9 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 11 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<article class = 'belowarticle'>
        <?php 
        $sql = "SELECT * FROM genserie WHERE genreid = 10 AND `type` = 'film'";
        if($result = $conn->query($sql)){
            while($row = $result-> fetch_array(MYSQLI_BOTH)){
                $sid = $row['serieid'];
                $sql1 = "SELECT * FROM films WHERE id = '$sid'";
...
<script>
    ...
    for(let i = 0; i < seriepopnum.length; i++){
        ...
        Seriepopnum.addEventListener("click", function() {
            ...
            $('#seriepop').load("includes/seriepop.inc.php?films=&id=" + id, {
                Id : id,
                Type: type
            }, function() {
